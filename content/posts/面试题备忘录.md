...
```
## de 和 se 的区别是什么
Data Engineer 解决的是 **数据如何产生、存储、加工、消费**。
Backend Engineer 解决的是 **应用如何提供功能和服务**。
技术栈不同，de 要保证数据质量、延迟、吞吐量、可追踪性，app/backend 关心业务逻辑、API、服务可靠性、用户体验。
Data Engineer：处理的是 TB～PB 级数据，App Engineer：处理的是 QPS、延迟、并发.
## 如何保证数据质量
4 个角度来：正确性（Correctness）、完整性（Completeness）、一致性（Consistency）、及时性（Timeliness）。

手段：
- Schema 管控（Schema Enforcement & Validation）
- 定义数据质量规则
- 监控与报警（Monitoring & Alerting）
- 数据血缘（Data Lineage）
- 幂等与容错（Idempotency & Fault Tolerance）
- 数据比对（Reconciliation / Discrepancy Check）
- 数据审计与异常恢复（Audit & Backfill）
## 你怎么提升自己的 engineer 水平的
code review, read books and tech blogs,