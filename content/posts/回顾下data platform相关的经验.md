...
过去几年我在不同规模、不同业务形态的公司做过数据平台相关工作，从在线特征服务到大型数据湖，再到自建数据库集群的创业公司。虽然技术栈差异巨大，但项目本质都是围绕同一件事：**让数据流动起来，让下游团队能用到可靠的数据**。

<!--more-->
## M 公司（日本 B2B 电商/以 BigQuery 为中心）

toC 业务，数据量没那么大，用 GCP 的下一些工具外加 python，go 完全足够。
...
## P 公司（大型金融科技 / 大数据平台与数据湖）

toC 业务，数据量大，需要用的一些大数据的技术栈。
...
创业公司，资金有限，用 aws infra 都基本以自建 k8s 集群，mysql 集群为主。
- 多 k8s 集群的 mysql 部署，用的是 statefulset，然后使用 aws volume 来作为 db 的存储卷，数据迁移的时候倒是挺方便，直接换 pv 就可以了
